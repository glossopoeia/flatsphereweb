...
> **Updated as of 07/09/2026** -- I submitted a PR on the PROJ repository with this optimization, where I learned from a review by Charles Karney that the closed form is already nicely formulated and explained by John P. Snyder in *Map Projections -- A Working Manual*. While the Aitoff projection is not mentioned by name in the text, the derivation is clear based on the construction of Aitoff in terms of the Azimuthal Equidistant projection, which is inverted on page 196. The Aitoff is just a pre- and post-scale of longitude on the Azimuthal Equidistant projection, so we just add some scalars to the relevant spots of the inverse formulas. Claude likely has this full text baked into it's training data, so it's maybe a little less impressive that it noticed this, but again, the fact that it's not directly stated as an Aitoff inverse in the text means the AI had to make a few connections on it's own to reach the conclusion. Thanks to Charles Karney for the detailed review which also ended up improving some of Flatsphere's formulations for Aitoff and Winkel Tripel.
