...
The Flatsphere SPA itself just needs any local web server (WebGPU requires a secure context). For SPA-only iteration:
...
The site also includes static pages (gallery, blog, merch, 404) generated by Jekyll. To preview those alongside the SPA, install the Ruby dependencies once and use the included Makefile:
...
`make serve` stages the projection data file Jekyll needs and runs `bundle exec jekyll serve --livereload` on `http://localhost:4000`. The SPA at `/` still works there too.

Requirements for the Jekyll path: Ruby 3.x and Bundler. The same Jekyll build runs in CI during deploys (see `.github/workflows/static.yml`).
...
- [Jekyll](https://jekyllrb.com) — Static site generator for the non-SPA pages (blog, gallery, merch, 404)
