...
The Flatsphere app itself just needs any local web server (WebGPU requires a secure context). For app-only iteration:
...
The site also includes static pages (gallery, blog, merch, 404) generated by Jekyll. To preview those alongside the app, install the Ruby dependencies once and use the included Makefile:
...
`make serve` stages the projection data file Jekyll needs and runs `bundle exec jekyll serve --livereload` on `http://localhost:4000`. The app at `/` still works there too.
